...
st.markdown(
    '<span class="pill">파스텔·미니멀</span>'
    '<span class="pill">관계 중심</span>'
    '<span class="pill">지도(버블)</span>',
    unsafe_allow_html=True,
)
st.markdown(
    '<div class="hint">관계를 또렷하게 보려면: <b>사분면</b>과 <b>지도</b>를 같이 보세요.</div>',
    unsafe_allow_html=True,
)
...
y_key = st.sidebar.selectbox(
    "Y(의료이용)",
    ["amount_per_capita", "patients_per_1k", "amount_per_patient", "total_amount_2024"],
    index=0,
)
...
df_rel = merged.copy()
df_rel = df_rel.loc[:, ~df_rel.columns.duplicated()]  # ✅ df_rel도 방어
...
with k1:
    kpi_card("시도 수", f"{df_rel['sido'].nunique():,}", "분석 대상")
with k2:
    kpi_card("인구증감 평균", f"{df_rel['pop_change_2024'].mean():,.0f} 명", "시도 평균")
with k3:
    kpi_card("1인당 의료비(중앙)", f"{df_rel['amount_per_capita'].median():,.0f} 원/인", "중앙값")
with k4:
    kpi_card("환자/1천명(중앙)", f"{df_rel['patients_per_1k'].median():,.1f}", "중앙값")
...
with tab1:
    ...
    with cB:
        ...
        show_cols = list(dict.fromkeys(show_cols))  # ✅ 중복 제거(순서 유지)

        top5 = merged.sort_values(y_key, ascending=False)[show_cols].head(5).copy()
        top5 = top5.loc[:, ~top5.columns.duplicated()]  # ✅ 혹시 모를 중복 방어
...
with tab2:
    ...
    cols = ["sido", x_key, y_plot, "patients_per_1k", "amount_per_capita"]
    cols = list(dict.fromkeys(cols))  # ✅ 중복 제거(순서 유지)

    q2 = q[q["quadrant"].str.startswith("Q2")][cols].copy()
    q2 = q2.loc[:, ~q2.columns.duplicated()]  # ✅ 혹시 남아있을 중복 방어

    st.dataframe(
        q2.sort_values(y_plot, ascending=False),
...
with tab3:
    ...
    map_metric = st.selectbox(
        "지도 색상 지표",
        ["amount_per_capita", "patients_per_1k", "pop_change_2024", "amount_per_patient"],
        index=0,
    )
    ...
    m = m.loc[:, ~m.columns.duplicated()]  # ✅ 방어
    ...
    fig.update_layout(
        ...
        title=f"지도: {label[map_metric]}",
    )
    st.plotly_chart(fig, use_container_width=True)
    st.caption(
        "※ 버블 맵은 시도 대표 좌표(대략)로 표시됩니다. "
        "행정경계(폴리곤) 지도도 원하면 GeoJSON 추가해서 바꿀 수 있어요."
    )
...
with tab4:
    ...
    z = z.loc[:, ~z.columns.duplicated()]  # ✅ 방어
...
with tab5:
    ...
    with c1:
        kpi_card("2024 인구증감 합계", f"{row['pop_change_2024']:,.0f} 명")
    with c2:
        kpi_card("1인당 의료비", f"{row['amount_per_capita']:,.0f} 원/인")
    with c3:
        kpi_card("인구 1천명당 환자수", f"{row['patients_per_1k']:,.1f}")
    with c4:
        kpi_card("환자 1인당 의료비", f"{row['amount_per_patient']:,.0f} 원/명")
    ...
    fig1 = px.line(
        d, x="month", y="pop_change", markers=True, title=f"{selected} 월별 인구증감(2024)", template="plotly_white"
    )
